...
void HCMAX7219::print7Seg_(char *TextString, unsigned int Offset)
{
  unsigned int _StringLength;
  unsigned int bufferindex;
  byte charindex;
  ...
  while(bufferindex != 0 && charindex != _StringLength)
  {
    if(TextString[charindex]==0x03)
     TextString[charindex]=0;
    DisplayBuffer[--bufferindex]=TextString[charindex++];
  } 
}
#endif
